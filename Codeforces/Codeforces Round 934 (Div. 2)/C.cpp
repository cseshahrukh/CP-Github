...
void solve(ll tt)
{
    // if there is single element, then both will try to pick minimum single element
    ll n;
    cin>>n; 
    ...
    for(int i=0; i<=2*10e5; i++)
    {
        ...
        if (counter[i] == 1 and alexpicked == false)
        {
            alexpicked = true;
        }
        else if (counter[i] == 1 and alexpicked == true)
        {
            cout<<i<<endl;
            return;
        }
