...
void solve(ll tt)
{
    // Corner Case of last submission 
    // 6
    // 11 0 99 10 46 78
    // here breakpoint is 99 and minval is 10
    // But when we face 0 we were not updating the minval 
    int n; 
    cin>>n; 
    ...
    // if a[i] > a[i-1] then ok
    // otherwise make a[i-1] split into digit and so on for all the digits to the left
    ...
    for (int i=n-1; i>=0; i--)
    {
        ...
        {
            breakpoint = i; 
            // cout<<"breakpoint is "<<arr[breakpoint]<<endl;
    ...
    for (int i=breakpoint; i>=0; i--)
    {
        ...
        while (temp)
        {
            ...
            minval=now; // updating the minval
        }
    }
